...
void Channels::Remove(const std::string& addr) {
    ...
    if(it == _channels.end()) {
        WARN("Channels::Remove failed, addr: {} not found", addr.c_str());
        return;
    }
...
void RpcManager::AddNode(const std::string& service_name, const std::string& addr) {
    ...
    if(channels == nullptr) {
        WARN("RpcManager::AddNode failed, service_name: {} not cared", service_name.c_str());
        return;
    }
...
void RpcManager::RemoveNode(const std::string& service_name, const std::string& addr) {
    Channels::Ptr channels = GetService(service_name);
    if(channels == nullptr) {
        WARN("RpcManager::RemoveNode failed, service_name: {} not cared", service_name.c_str());
        return;
    }
...
ChannelPtr RpcManager::GetNode(const std::string& service_name) {
    Channels::Ptr channels = GetService(service_name);
    if(channels == nullptr) {
        WARN("RpcManager::GetNode failed, service_name: {} not cared", service_name.c_str());
        return nullptr;
    }
...
std::shared_ptr<brpc::Server> ServerFactory::Create(int port, google::protobuf::Service* service) {
    ...
    if (server->AddService(service, brpc::SERVER_OWNS_SERVICE) != 0) {
        ERROR("ServerFactory::Create failed, add service to server failed");
        return nullptr;
    }
    brpc::ServerOptions options;
    options.idle_timeout_sec = -1; // 连接最大空闲时间，单位秒。设置为-1表示不关闭空闲连接
    if (server->Start(port, &options) != 0) {
        ERROR("ServerFactory::Create failed, start server on port: {} failed", port);
        return nullptr;
    }
    return server;
}
